...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeEmCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeEmCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeEmCoinAmount = strAnonymizeEmCoinAmount.remove(strAnonymizeEmCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEmCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeEmCoinAmount*COIN) nMaxToAnonymize = nAnonymizeEmCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeEmCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeEmCoinAmount));
        strAnonymizeEmCoinAmount = strAnonymizeEmCoinAmount.remove(strAnonymizeEmCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEmCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
