...
bool CMasternodeConfig::read(boost::filesystem::path path) {
    ...
    for(std::string line; std::getline(streamConfig, line); )
    {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex >> donation)) {
            ...
            if (!address.IsValid()) {
                LogPrintf("InvalidEMCaddress in masternode.conf line: %s\n", line.c_str());
                streamConfig.close();
                return false;
            }
        }
        ...
        if(Params().NetworkID() == CChainParams::MAIN){
            if(CService(ip).GetPort() != 21507) {
                LogPrintf("Invalid port detected in masternode.conf: %s (must be 21507 for mainnet)\n", line.c_str());
                streamConfig.close();
                return false;
            }
        } else if(CService(ip).GetPort() == 21507) {
            LogPrintf("Invalid port detected in masternode.conf: %s (21507 must be only on mainnet)\n", line.c_str());
            streamConfig.close();
            return false;
        }
