    ...
    model(0)
{
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "EMC(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "EMC(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(EMC)"<<"TOTAL COST(BTC");
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://future-exchange.com/api/v1.1/public/GetMarketSummary?market=btc-EMC");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://future-exchange.com/api/v1.1/public/getorderbook?market=BTC-EMC&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://future-exchange.com/api/v1.1/public/getmarkethistory?market=BTC-EMC&count=100");
      return Response;
}
...
QString tradingDialog::BuyTX(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://future-exchange.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-EMC&quantity=";
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://future-exchange.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-EMC";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://future-exchange.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=EMC";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://future-exchange.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-EMC&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>EMC Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> EMC");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->TXSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(TXSupply,'i',8) + "</span><b> EMC</b>");
    ...
    ui->TXDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(TXDemand,'i',8) + "</span><b> EMC</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("EMC");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->TXBalanceLabel,*ui->TXAvailableLabel_2,*ui->TXPendingLabel, QString::fromUtf8("EMC"),Response);
                    }
                break;
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("EMC");
...
{
    //calculate amount of BTC that can be gained from sellingEMCavailable balance
    QString responseA = GetBalance("EMC");
    QString str;
                ...
                if (SellResponseObject["success"].toBool() == false){
                    ...
                    if (ResponseObject["success"].toBool() == false){
                        ...
                        if (ResponseObject["success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+"EMCfor "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+"EMCfor "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
