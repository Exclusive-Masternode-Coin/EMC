...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <emcaddress> <amount>\n"
            "emcaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <emcaddress> <amount>\n"
            "emcaddress, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in emc.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in emc.conf\n"
                ...
                "  vote-many    - Vote on a EMC initiative\n"
                "  vote         - Vote on a EMC initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CEMCSecret(secret).ToString();
    }
    ...
    if(strCommand == "status")
    {
        ...
        if (pmn) mnObj.push_back(Pair("pubkey", CEMCAddress(pmn->pubkey.GetID()).ToString()));
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a EMC initiative (can be additionally filtered, partial match)\n"
